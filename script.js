...
async function fetchPokemonById(id) {
  ...
  showLoading = true;
  // 2.1)
  const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`); //this id is a variable within a string thats gonna read as string and the way we write is ${variable}
...
async function loadQuestionWithOptions() {
  ...
  // 9) Create/reset the options array with the correct answer (pokemon.name)
  ...
  optionsContainer.innerHTML = ""; // an empty string for buttons..Reset
...
function checkAnswer(isCorrect, event) {
  // 15.1) Check if any button is already selected, if false => no element => "null".
  ...
  if (isCorrect) {
    ...
    // 15.8) If correct increase the points by 1 also changes color to green by var corect in css
    points++;
    pointsElement.textContent = points;
    event.target.classList.add("correct");

    // similarly changes to var wrong in css whose color is red
  } else {
